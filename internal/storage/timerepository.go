...
func (storage *Storage) CreateList() error {
	createTebale, err := storage.conn.Exec(context.Background(), createTable)
	if err != nil {
...
func (storage *Storage) UpdateTimer(start, finish time.Time, name string) (*model.Timer, error) {
	_, err := storage.conn.Exec(context.Background(), "UPDATE timer set name=$1, start_time=$2, finish_time=$3", start, finish, name)
	if err != nil {
